...
def to_excel(df):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        # Dynamic Auto-Width and Numeric Formatting
    ...
    if all_data:
        ...
        with c1:
            fig = px.pie(df, names='Section', values='Tax (₹)', 
                         title="Tax by Section", hole=0.4,
                         color_discrete_sequence=px.colors.qualitative.Prism)
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", font=dict(color="white"))
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            st.markdown('<div class="zone">📥 Audit Report Ready (Statutory Compliant)</div>', unsafe_allow_html=True)
            st.download_button("🚀 Download Professional Excel Audit", data=to_excel(df), file_name="TDS_Challan_Audit_2026.xlsx")

        st.dataframe(df.style.format({"Tax (₹)": "{:,.2f}", "Total Paid (₹)": "{:,.2f}"}), use_container_width=True)
    else:
        st.error("❌ No valid patterns found. If this is a scanned PDF, please use OCR.")
